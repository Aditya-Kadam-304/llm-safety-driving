...
Changes from sensor_code_13.py (thesis-readiness improvements):
 18) REPRODUCIBILITY: RNG re-seeded per (scenario, iteration) for deterministic results.
     Formula: np.random.seed(base_seed + hash(scenario_id) % 2**31 + iter_num)
 19) CSV: New 'iteration' column tracks which iteration produced each row.
 20) CSV: New 'llm_action', 'llm_speed', 'llm_pass' columns record the LLM's
     pre-cage proposal — enables measuring LLM accuracy independently of SafetyCage.
 21) CSV: New 'llm_rationale' column records the LLM's reasoning string (truncated).
 22) METADATA: run_suite() saves metadata.json at the start of each run with:
     code version, Python/torch/transformers versions, GPU model, seed, model_id,
     quantization config, max_new_tokens, and timestamp.
 23) STATISTICS: summarize_results() now computes:
     - 95% Wilson score confidence intervals for accuracy
     - Per-iteration accuracy breakdown
     - McNemar's test for pairwise model comparison (exact p-values via scipy)
     - Precision, Recall, F1-Score, Specificity per model × config
 24) DOCSTRINGS: Added comprehensive docstrings to oracle_expected() and run_one().
 25) CONSTANTS: Threshold constants table at module top for easy auditing.
 26) PLOTS: 16 publication-ready plots including confusion matrices and latency
     distribution (violin+box) in addition to ablation, radar, heatmap, etc.
 27) LATEX: Auto-exports results_table.tex with multi-model and ablation tables
     for direct \\input{} inclusion in thesis chapters.
 28) CSV ROBUSTNESS: Uses Python csv module (csv.writer/csv.reader) for proper
     quoting instead of manual comma-join.

Previous changes (sensor_code_12 → sensor_code_13):
 13-17) KV-cache, multi-model CLI, model_name CSV column, multi-model plots.

Previous changes (sensor_code_11 → sensor_code_12):
 10-12) IMU/GPS sensors, fault injection, Template F scenarios.

Previous changes (sensor_code_10 → sensor_code_11):
  8-9) Template E sensor degradation, BIST-specific faults.

Previous changes (sensor_code_9 → sensor_code_10):
  5-7) Template D ambiguous scenarios, expanded oracle, RuleAdvisor baseline.

Previous changes (from sensor_code_8→9):
  1-4) Shadow LLM reuse, SafetyCage physics, CSV run_ts, oracle symmetric pass/fail.

Key design decisions (per your thesis meeting):
- FINAL mode = real pipeline + safety cage final authority.
- SHADOW is embedded in FINAL (LLM runs even when "all good" to log comparison, but doesn't override).
- ABLATE is separate mode (component-off experiments) to prove influence scientifically.
